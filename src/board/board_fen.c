...
int _get_piece_placement(const board_t *board, char *fen);
int _get_side_to_move(const board_t *board, char *fen);
int _get_castling_rights(const board_t *board, char *fen);
int _get_en_passant_square(const board_t *board, char *fen);
int _get_halfmove_clock(const board_t *board, char *fen);
int _get_fullmove_number(const board_t *board, char *fen);

void board_get_fen(const board_t *board, char *fen)
{
	int i = 0;

	i += _get_piece_placement(board, fen);
	fen[i++] = SPACE;

	i += _get_side_to_move(board, fen + i);
	fen[i++] = SPACE;

	i += _get_castling_rights(board, fen + i);
	fen[i++] = SPACE;

	i += _get_en_passant_square(board, fen + i);
	fen[i++] = SPACE;

	i += _get_halfmove_clock(board, fen + i);
	fen[i++] = SPACE;

	i += _get_fullmove_number(board, fen + i);
	fen[i++] = TERMINATOR;
}

int _set_piece_placement(board_t *board, const char *fen);
int _set_side_to_move(board_t *board, const char *fen);
int _set_castling_rights(board_t *board, const char *fen);
int _set_en_passant_square(board_t *board, const char *fen);
int _set_halfmove_clock(board_t *board, const char *fen);
int _set_fullmove_number(board_t *board, const char *fen);
...
void board_set_fen(board_t *board, const char *fen)
{
	...
	i += _set_castling_rights(board, fen + i);
	i++;

	i += _set_en_passant_square(board, fen + i);
	i++;

	i += _set_halfmove_clock(board, fen + i);
	i++;

	i += _set_fullmove_number(board, fen + i);
	i++;
}
