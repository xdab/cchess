...
void check_regular_move(board_t *board)
{
	...
	if (board_get(board, E4) != (PAWN | WHITE))
	...
	board_unmove(board);
	if (board_get(board, E2) != (PAWN | WHITE))
...
{
	board_init(board);
	board_move(board, move_regular(E2, E4)); // e4
	board_move(board, move_regular(D7, D5)); // d5

	board_move(board, move_regular(E4, D5)); // exd5
	if (board_get(board, D5) != (PAWN | WHITE))
	...
	if (board_get(board, D5) != (PAWN | BLACK))
...
{
	board_init(board);
	board_move(board, move_regular(C2, C4));		   // 1. c4
	board_move(board, move_regular(C7, C5));		   // 1. ... c5
	board_move(board, move_regular(D2, D4));		   // 2. d4
	board_move(board, move_regular(C5, D4));		   // 2. ... cxd4
	board_move(board, move_regular(C4, C5));		   // 3. c5
	board_move(board, move_regular(D7, D6));		   // 3. ... d6
	board_move(board, move_regular(C5, C6));		   // 4. c6
	board_move(board, move_regular(C8, D7));		   // 4. ... Bd7
	board_move(board, move_regular(C6, C7));		   // 5. c7
	board_move(board, move_regular(D7, E6));		   // 5. ... Be6
	board_move(board, move_promotion(C7, C8, QUEEN)); // 6. c8=Q
	if (board_get(board, C8) != (QUEEN | WHITE))
	...
	board_unmove(board);
	if (board_get(board, C7) != (PAWN | WHITE))
